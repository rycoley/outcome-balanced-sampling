### RED Suicide ANN Simulation Function
...
###    batch_size            Batch size within each epoch for unbalanced models
###    mini_n_controls       Number of controls per mini-batch  (balanced sampling)
###    mini_n_cases          Number of cases per mini-batch (balanced sampling) 


red.sim.nn <- function(sim_replicate, dat_s, n_val_rep, event_rate, n_sim_train, n_sim_test, n_sim_val, 
                    qtile_probs_test, num_cols_valid, nboot, nodes, fname.boot, fname.results,
                    n_epochs, batch_size=NULL, w_decay = 0.0001, mini_n_controls=NULL, mini_n_cases=NULL) {
  ### Generate data
  ...
  if(!is.null(batch_size)) {
    ...
    modelfit.time <- system.time({
      nn_predictions <- fit_nn(train=sim_data$dat_train, test=sim_data$dat_test, 
                               valid=sim_data$dat_val[, colnames(sim_data$dat_train)], 
                               test_race=sim_data$race_cat_test, train_race=sim_data$race_cat_train, 
                               valid_race=sim_data$race_cat_val,
                               n_epochs=50, batch_size=batch_size, lr, w_decay = 0.0001)
    })
    
  } else if (!is.null(mini_n_controls) & !is.null(mini_n_cases)) {
    
    modelfit.time <- system.time({
      nn_predictions <- fit_nn_mini(train=sim_data$dat_train, test=sim_data$dat_test, 
                                    valid=sim_data$dat_val[, colnames(sim_data$dat_train)], 
                                    test_race=sim_data$race_cat_test, train_race=sim_data$race_cat_train, 
                                    valid_race=sim_data$race_cat_val, n_epochs=n_epochs, 
                                    mini_n_controls=mini_n_controls, mini_n_cases=mini_n_cases, 
                                    lr, w_decay = 0.0001)
    })
  ...
  preds_train_sample <- nn_predictions$preds_train_sample
  # predictions on the test set
  preds_test_sample <- nn_predictions$preds_test_sample
  # predictions on the validation set
  preds_val_sample <- nn_predictions$preds_val_sample
  
  train_y <- nn_predictions$train_y
  test_y <- nn_predictions$test_y
  valid_y <- nn_predictions$valid_y
  train_x <- nn_predictions$train_x
  ...
  rm(results_list, point.estimates,validation.point.estimates,marginal_mean_test_calib,marginal_mean_valid_calib, nn_predictions,
