### RED Suicide RF Simulation Function
...
red.sim.rf <- function(sim_replicate, dat_s, n_val_rep, event_rate, n_sim_train, n_sim_test, n_sim_val, 
                    qtile_probs_test, num_cols_valid, nboot, nodes, fname.boot, fname.results,
                    rate_difference, mtry, minimum_node_size, tree_count) {
  ### Generate data
  ...
  rf_predictions <- fit_rf(sim_data$dat_train, sim_data$dat_test, sim_data$dat_val, 
  ...
  preds_train_sample <- rf_predictions$preds_train_sample
  # predictions on the test set
  preds_test_sample <- rf_predictions$preds_test_sample
  # predictions on the validation set
  preds_val_sample <- rf_predictions$preds_val_sample
  
  train_y <- rf_predictions$train_y
  test_y <- rf_predictions$test_y
  valid_y <- rf_predictions$valid_y
  train_x <- rf_predictions$train_x
  ...
  rm(results_list, point.estimates,validation.point.estimates,marginal_mean_test_calib,marginal_mean_valid_calib, rf_predictions,
