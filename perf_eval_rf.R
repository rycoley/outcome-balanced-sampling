...
# 1:1 Subsample 
fname.boot <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/boot_indices/RF/1_1/sim_"
fname.results <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/1_1/sim_"
rate_difference <- 1
...
total.runtime <- system.time({
  for(iteration in 1:sim_replicate){
    red.sim.rf(sim_replicate=iteration, dat_s, n_val_rep, event_rate, n_sim_train, n_sim_test, n_sim_val, 
               qtile_probs_test, num_cols_valid, nboot, nodes, fname.boot, fname.results, rate_difference,
               mtry, minimum_node_size, tree_count)
  }
  
})

params <- c(sims=sim_replicate, rate=event_rate, n_train=n_sim_train, n_test=n_sim_test, n_val=n_sim_val, 
            nboot=nboot, nodes=nodes, rate_diff=rate_difference,
            mtry=mtry, min_node_size=minimum_node_size, tree_count=tree_count)

rf_1_1 <- format_simulations(max_iter = sim_replicate, rate = rate_difference, 
                             model = "RF", digits = 5, folder_path = fname.results) 

# 1:2 Subsample 
rm(total.runtime)
fname.boot <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/boot_indices/RF/1_2/sim_"
fname.results <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/1_2/sim_"
rate_difference <- 2
minimum_node_size <- 25
tree_count <- 250
...
rf_1_2 <- format_simulations(max_iter = sim_replicate, rate = rate_difference, model = "RF", digits = 5, folder_path = fname.results) 

# 1:5 Subsample
rm(total.runtime)
fname.boot <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/boot_indices/RF/1_5/sim_"
fname.results <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/1_5/sim_"
rate_difference <- 5
minimum_node_size <- 50
tree_count <- 100
...
rf_1_5 <- format_simulations(max_iter = sim_replicate, rate = rate_difference, model = "RF", digits = 5, folder_path = fname.results) 

# Full Sample
rm(total.runtime)
fname.boot <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/boot_indices/RF/control/sim_"
fname.results <- "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/control/sim_"
rate_difference <- NA 
mtry <- 12
...
sim_replicate = length(list.files("G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/control/")) 
sim_replicate = 101
rf_control <- format_simulations(max_iter = sim_replicate, rate = rate_difference, model = "RF", digits = 5, folder_path = fname.results)


rf_1_1_val_cal = marg_val_calib(rf_1_1)
rf_1_2_val_cal = marg_val_calib(rf_1_2)
rf_1_5_val_cal = marg_val_calib(rf_1_5)
rf_control_val_cal = marg_val_calib(rf_control)

write.table(cbind(rf_1_1_val_cal, rf_1_2_val_cal, rf_1_5_val_cal, rf_control_val_cal), "clipboard", sep = "\t")

rf_results <- rbind(cbind(rf_1_1$results_table, sample= "1_1", model="rf"), 
                     cbind(rf_1_2$results_table, sample= "1_2", model="rf"), 
                     cbind(rf_1_5$results_table, sample= "1_5", model="rf"), 
                     cbind(rf_control$results_table, sample= "1_c", model="rf"))

write.csv(rf_results, 
          file = "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/rf_final_results.csv")
...
metric <-  names(table(rf_1_1$results_table$metric))
strata <- names(table(rf_1_1$results_table$strata))

results_rf <- do.call(bind_rows, lapply(paste0("rf_", c("1_1", "1_2", "1_5", "control")), function(v) {
...
saveRDS(results_rf, 
        file = "G:/CTRHS/RED_suicide_prevention/ANALYSIS/freddy/outcome_sampling/sim_rep_results/RF/rf_paper_results.RDS")
